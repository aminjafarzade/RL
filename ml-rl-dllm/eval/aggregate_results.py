...
def aggregate_results(results_dir):
    ...
    for json_file in evalpy_files:
        try:
            ...
            avg_verifier_calls = (
                sum(item.get("verifier_calls", 0) for item in data.get("generations", []))
                / total_processed
                if total_processed > 0
                else 0
            )
            avg_remask_count = (
                sum(item.get("remask_count", 0) for item in data.get("generations", []))
                / total_processed
                if total_processed > 0
                else 0
            )
...
def create_summary_tables(df, output_dir):
    ...
    agg_dict["avg_verifier_calls"] = ["mean", "std", "min", "max"]
    agg_dict["avg_remask_count"] = ["mean", "std", "min", "max"]
    agg_dict["parseable_rate"] = ["mean", "std", "min", "max"]
